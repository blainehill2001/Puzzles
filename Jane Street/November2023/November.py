...
def simulate_waiting(i, j, time, lattice, max_time=180):
    ...
    if lattice[rising_coord[0], rising_coord[1]] == lattice[i,j]:
        rising_equals = True
        sinking_coords.remove(rising_coord) #this works even if rising_coord wasn't in sinking_coords
    else:
       rising_equals = False
    for time_spent_waiting in range(0, 6*len(sinking_coords), len(sinking_coords)):
        timed_lattice = lattice.copy()
        
        for x, y in sinking_coords:
            timed_lattice[x, y] -= time_spent_waiting * 1.0 / (
                len(sinking_coords) 
            )
            #timed_lattice[x, y] -= 1
...
def find_max_time_iterative(start_i, start_j, start_time, start_lattice, start_visited, start_path):

    queue = deque([(start_i, start_j, start_time, start_lattice, start_visited, start_path)]) 
    #   max_path, max_time = [], 0
    ...
    while queue:
        ...
        for di, dj, dz in possible_moves:
            ...
            if 0 <= ni <= 7 and 0 <= nj <= 7 and visited[ni][nj] < 3:
                ...
                for timed_lattice, wait_time in simulate_waiting(i, j, time, lattice):
                    ...
                    if x > y: continue_sinkining=True #if the spot the knight with the change in height is on is still higher than where it is trying to go
                    if abs(x - y) > 1e-5: 
                        continue
                    ...
                    queue.append((ni, nj, time + wait_time, timed_lattice, cur_visited , cur_path))
...
max_path = list(map(lambda x: (x[0],convert_to_lattice_point_notation(x[1])), max_path))
