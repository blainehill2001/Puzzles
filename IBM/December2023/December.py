...
def generate_isometric_grid_equations(m):
    ...
    for k in range(2*m):
        horizontal_equation_upwards = lambda x, k=k: k * sqrt(3) / 2
        # horizontal_equation_downwards = lambda x, k=k: -k * sqrt(3) / 2  #won't need for first quadrant analysis
        positive_diagonal_equation_forwards = lambda x, k=k: sqrt(3) * x + k * sqrt(3)
        positive_diagonal_equation_backwards = lambda x, k=k: sqrt(3) * x - k * sqrt(3)
        negative_diagonal_equation_forwards = lambda x, k=k: -sqrt(3) * x + k * sqrt(3)
        # negative_diagonal_equation_backwards = lambda x, k=k: -sqrt(3) * x - k * sqrt(3) #won't need for first quadrant analysis
        equations.extend([horizontal_equation_upwards, positive_diagonal_equation_forwards, positive_diagonal_equation_backwards, negative_diagonal_equation_forwards])
...
def visualize_three_line_segments(segs):
    ...
    x1, x2, x3, y1, y2, y3 = [x1,x2], [x3,x4], [x5,x6], [y1,y2], [y3,y4], [y5,y6]
...
def check_parallel_lines(segs):
    ...
    return slopes.count(slopes[0]) >= 2 or slopes.count(slopes[1]) >= 2 or slopes.count(slopes[2]) >= 2
    
def find_line_seg_intersection_points(line_segments, m, log=True):

    dir_path = f'./line seg intersection points'
    ...
    if not os.path.exists(file_path):
        ...
        for ((x1, y1), (x2, y2)), ((x3, y3), (x4, y4)), ((x5, y5), (x6, y6)) in tqdm(triplets, desc="Processing triplets"):
                    ...
                    if (
                        intersection_point and
                        (x1 < intersection_point[x] < x2) and
                        (y1 < intersection_point[y] < y2) and
                        (x3 < intersection_point[x] < x4) and
                        (y3 < intersection_point[y] < y4) and
                        (x5 < intersection_point[x] < x6) and
                        (y5 < intersection_point[y] < y6)
                    ):
                        print("\n\n\n\n")
                        print("GOT HERE!!!!!!!!!!")
                        print(intersection_point)
                    ...
                    if log:
                        ...
                        with open('log.txt', 'a') as f:
                            f.write(str((intersection_point, ((x1, y1), (x2, y2)), ((x3, y3), (x4, y4)), ((x5, y5), (x6, y6)))))
                            f.write('\n')
                        if i == important_val:
                            visualize_three_line_segments((((x1, y1), (x2, y2)), ((x3, y3), (x4, y4)), ((x5, y5), (x6, y6))))
...
def get_line_segments(m):
    ...
    if not os.path.exists(file_path):
        ...
        for equation in tqdm(equations, desc="Processing equations"):
            ...
            small_circle_eqn = lambda x: sqrt(m**2 - x**2)
            large_circle_eqn = lambda x: sqrt((m+1)**2 - x**2)
            smaller_intersection_points = find_intersection_points(equation, small_circle_eqn)
            larger_intersection_points = find_intersection_points(equation, large_circle_eqn)

            # Display the intersection points
            for x_small, x_large in zip(smaller_intersection_points, larger_intersection_points):
                line_segments.append(((x_small, equation(x_small)),(x_large, equation(x_large))))
...
def solve(m, graph=False):
    ...
    count, unique_coordinates = count_and_get_coordinates(intersection_points, m)
    ...
    if graph:
        ...
        small_circle = patches.Circle((0, 0), radius=m, edgecolor='b', facecolor='none')
        ax.add_patch(small_circle)
        ax.set_aspect('equal', adjustable='box')
        ax.add_artist(small_circle)

        large_circle = patches.Circle((0, 0), radius=m+1, edgecolor='b', facecolor='none')
...
def binary_search_for_puzzle_answer(start_interval, end_interval):
    ...
    while start_interval < end_interval:
        ...
        with open('answer.txt', 'w') as f:
                f.write(f"The number of non integer radii circles between {m} and {m+1}: {ans}")
        if cur_ans == 1_000_000:
            with open('answer.txt', 'w') as f:
                f.write(f"The answer to this month's puzzle (the m that yields 1 million) is: {m}")
            return m
